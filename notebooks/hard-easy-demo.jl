...
function visualize_trial_pair(a::WorldState, b::WorldState, steps = 100)
    ...
    for t = 1:steps
        ...
        visuals[t] = hcat(paint_state(a, wm), paint_state(b, wm))
    end
    return visuals
end;
...
function init_trial(hard=true)
	ylwy = hard ? 20 : -20
	static = StaticState([
		StaticObject(S2V(  0, -150), Rectangle(50, 10, 0.),   1.0),   # Red
		StaticObject(S2V(170,  -80), Rectangle(10, 50, 0.), 110.0),   # Green
		StaticObject(S2V(130, ylwy), Rectangle(10, 90, 0.),  60.0),   # Yellow
		])
...
function init_agent(istate::WorldState)
	...
	ac = AdaptiveComputation(;
							 nns=10, buffer_size=500, base_steps=4, load=10, 
...
function run_agent(istate::WorldState, steps = 50)
...
combined = map(x -> hcat(x...), zip(hard_sim, easy_sim));
